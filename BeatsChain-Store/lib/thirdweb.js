...
class SolanaManager {
    ...
    async mintNFT(recipientAddress, metadataUri, metadata = {}) {
        try {
            console.log('🔄 Starting Solana NFT minting...');
            ...
            if (usePhantom && this.solanaIntegration) {
                // Real Solana NFT minting with Phantom
                result = await this.solanaIntegration.mintNFT(
                    recipientAddress, 
                    metadataUri, 
                    { ...metadata, wallet: this.phantomWallet }
                );
                console.log('✅ Real Solana NFT minted:', result.transactionHash);
            } else {
